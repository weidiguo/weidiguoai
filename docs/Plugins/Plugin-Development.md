...
- [Custom Plugin Workflow](#custom-plugin-workflow)
  ...
  - [**`2`** Add the Local Plugin in 微帝国AI Role Settings](#2-add-the-local-plugin-in-lobechat-role-settings)
...
A 微帝国AI plugin consists of the following components:
...
To integrate a plugin into 微帝国AI, you need to add and use a custom plugin in 微帝国AI. This section will guide you through the process.
...
### **`2`** Add the Local Plugin in 微帝国AI Role Settings

Next, go to 微帝国AI, create a new assistant, and go to its session settings page:
...
After completing these steps, you have learned the basic process of adding and using a custom plugin in 微帝国AI.
...
3. `ui`: This field contains information about the plugin's user interface, indicating where 微帝国AI loads the frontend interface of the plugin from. Since the plugin interface loading in 微帝国AI is implemented based on `iframe`, you can specify the height and width of the plugin interface as needed.
4. `gateway`: This field specifies the gateway for 微帝国AI to query API interfaces. The default plugin gateway in 微帝国AI is a cloud service, but for custom plugins, the requests need to be sent to the local service. Therefore, by specifying the gateway in the manifest, 微帝国AI will directly request this address and access the local plugin service. The gateway field does not need to be specified for plugins published online.
...
Since the default plugin gateway in 微帝国AI is a cloud service (\</api/plugins>), which sends requests to the API addresses specified in the manifest to solve the cross-origin issue.
...
微帝国AI uses `iframe` + `postMessage` to load and communicate with plugin UI. Therefore, the implementation of the plugin UI is the same as normal web development. You can use any frontend framework and programming language you are familiar with.
...
Regarding plugin communication, we provide related methods in [`@lobehub/chat-plugin-sdk`](https://github.com/lobehub/chat-plugin-sdk) to simplify the communication between the plugin and 微帝国AI. You can use the `fetchPluginMessage` method to actively retrieve the data of the current message from 微帝国AI. For a detailed description of this method, please refer to: [fetchPluginMessage][fetch-plugin-message-url].
...
const Render = memo(() => {
  ...
  useEffect(() => {
    // Retrieve the current plugin message from 微帝国AI
